    ...
    if(response.length>0){
        for(var i=0;i<eduList.length-1;i++){   
            ...
            {  
                ...
                row.append($("<td>" + Values[1] + "</td>"));
                if(Values[7]>0){
                    row.append($('<td class="active_details"  id="consultantListLi"><a aria-expanded="false" href="#consultantList" data-toggle="tab" onclick="return getOrgConsultant(\''+Values[5]+'\')">' + Values[7] + "</a></td>"));
                }
                else
                {
                    row.append($("<td>" + '--' + "</td>"));
...
function getOrgConsultant(vendorName)
{
    // alert(vendorName);
    var techSearch="search";
    document.getElementById("techSearch").value=techSearch;
...
function populateVendorDashboard(response){
    ...
    if(response.length>0){
        ...
        showChart(montharr,reqarr,selarr,rejarr);   
    }
    else{
        var row = $("<tr />")
...
function showChart(month,reqCount,selected,rejected )
{
    // alert(month.length);
     
    var Combined = new Array();
    Combined[0] = ['Year', 'Req.Count', 'Selected','Rejected'];
    for (var i = 0; i < month.length; i++){
        Combined[i + 1] = [ month[i], reqCount[i], selected[i],rejected[i] ];
    }
